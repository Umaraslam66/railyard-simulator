...
# Function to create a quadratic Bézier curve (clean arc-like curve)
def create_quadratic_bezier(x1, y1, x2, y2, control_offset):
    # Control point (midpoint between x1, y1 and x2, y2 with added offset)
    control_x = (x1 + x2) / 2 + control_offset[0]
    control_y = (y1 + y2) / 2 + control_offset[1]
    
    t_values = np.linspace(0, 1, 100)  # Generate 100 points along the curve
...
def create_mixed_edge(x1, y1, x2, y2, straight_length, control_offset, angle=None, curve_first=False):
    if curve_first:
        ...
        # Calculate the intersection point of the curve and the straight line
        ...
        # Combine the curve and straight parts
        xs = list(bezier_xs) + straight_xs
        ys = list(bezier_ys) + straight_ys
    else:
        # Straight line followed by a curve
        # Calculate the end point of the straight part
        if angle is None:  # Default to a straight line directly towards the second node
        ...
        # Combine the straight and curve parts
        xs = straight_xs + list(bezier_xs)[1:]  # Exclude the first point of the curve to avoid duplication
        ys = straight_ys + list(bezier_ys)[1:]
    
    return xs, ys

# Add edges with lengths, using polyline, Bézier curve, or mixed straight-curve based on the node pair
for node1, node2, length in edges_with_lengths:
    if node1 in coordinates and node2 in coordinates:
        x1, y1 = coordinates[node1]
        x2, y2 = coordinates[node2]
        
        if (node1, node2) in polyline_edges:  # Check if this edge needs a polyline
            segments = polyline_edges[(node1, node2)]['segments']
            offset = polyline_edges[(node1, node2)]['offset']
            xs, ys = create_polyline(x1, y1, x2, y2, segments, offset)
            edge_color = polyline_color
            line_style = dict(width=3, dash='dash')
        
        elif (node1, node2) in curve_edges:  # Check if this edge needs a curve
            control_offset = curve_edges[(node1, node2)]['control_offset']
            xs, ys = create_quadratic_bezier(x1, y1, x2, y2, control_offset)
            edge_color = curve_color
            line_style = dict(width=3, dash='dot')
        
        elif (node1, node2) in mixed_edges:  # Check if this edge is a mixed curve-straight
            straight_length = mixed_edges[(node1, node2)]['straight_length']
            control_offset = mixed_edges[(node1, node2)]['control_offset']
            angle = mixed_edges[(node1, node2)].get('angle')  # Get the angle if specified
            curve_first = mixed_edges[(node1, node2)].get('curve_first', False)  # Default to straight line first if not specified
        ...
        else:  # Default to a straight line if no polyline, curve, or mixed edge is specified
            xs, ys = [x1, x2], [y1, y2]
            edge_color = 'black'
